...
- `QUIZ_ARCHIVER_PROXY_SERVER_URL`: URL of the proxy server to use for all playwright requests. HTTP and SOCKS proxies are supported. If not set, auto-detection will be performed. If set to false, no proxy will be used. (default=`None`)
- `QUIZ_ARCHIVER_PROXY_USERNAME`: Optional username to authenticate at the proxy server (default=`None`)
- `QUIZ_ARCHIVER_PROXY_PASSWORD`: Optional password to authenticate at the proxy server (default=`None`)
