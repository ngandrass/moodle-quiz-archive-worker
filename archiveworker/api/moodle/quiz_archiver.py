...
class QuizArchiverMoodleAPI(MoodleAPIBase):
    ...
    def get_backup_status(
        ...
        try:
            ...
            r = self.session.get(
                ...
                timeout=self.REQUEST_TIMEOUTS,
                params=self._generate_wsfunc_request_params(
                    wsfunction=self.MOODLE_WSFUNCTION_GET_BACKUP,
                    jobid=str(jobid),
        ...
        if response['status'] == MoodleBackupStatus.SUCCESS:
            return MoodleBackupStatus.SUCCESS

        if response['status'] == MoodleBackupStatus.FAILED:
            return MoodleBackupStatus.FAILED
    ...
    def get_attempts_metadata(
        ...
        params = self._generate_wsfunc_request_params(
            ...
            jobid=str(jobid),
            courseid=jobdescriptor.courseid,
            cmid=jobdescriptor.cmid,
            quizid=jobdescriptor.quizid
        )
        ...
        for batch in batches:
            ...
            if not (
                data['courseid'] == jobdescriptor.courseid and
                data['cmid'] == jobdescriptor.cmid and
                data['quizid'] == jobdescriptor.quizid
            ):
                raise ValueError(f'Moodle webservice function {self.MOODLE_WSFUNCTION_GET_ATTEMPTS_METADATA} returned an invalid response')
    ...
    def get_attempt_data(
        ...
        if not (
                ...
                data['courseid'] == jobdescriptor.courseid and
                data['cmid'] == jobdescriptor.cmid and
                data['quizid'] == jobdescriptor.quizid and
    ...
    def process_uploaded_artifact(
        ...
        if 'exception' in response and 'message' in response:
            raise RuntimeError(f'Moodle webservice function {self.MOODLE_WSFUNCTION_PROESS_UPLOAD} returned exception "{response["exception"]}". Message: {response["message"]}')
